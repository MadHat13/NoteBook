      ...
      <div style="width:1000000px;">
        <div id="catalogue" :style="catalogue_attribute">
          <ul class="catalogue-ul">
            <li
              :class="{'catalogue-li':true,'catalogue-li-click':catalogue_ul[0].isClick}"
              @click="turnBlue(catalogue_ul[0].id),newdata()"
            >{{catalogue_ul[0].text}}</li>
            <li
              :class="{'catalogue-li':true,'catalogue-li-click':catalogue_ul[1].isClick}"
              @click="turnBlue(catalogue_ul[1].id)"
            >{{catalogue_ul[1].text}}</li>
            <ul v-if="catalogue_ul[1].isClick" id="titlelist">
              <li
                v-for="title in my_content"
                :key="title.dataid"
                @click="showcontent(title.dataid)"
              >{{title.title}}</li>
            </ul>
            <li
              :class="{'catalogue-li':true,'catalogue-li-click':catalogue_ul[2].isClick}"
              @click="turnBlue(catalogue_ul[2].id)"
            >{{catalogue_ul[2].text}}</li>
            <ul v-if="catalogue_ul[2].isClick" id="titlelist">
              <li
                v-for="title in trash_content"
                :key="title.dataid"
                @click="showcontent(title.dataid)"
              >{{title.title}}</li>
            </ul>
        ...
        <div id="searchbox" :style="search_attribute">
          ...
          <div class="searchbox-list">
            <ul class="searchbox-list-ul">
              <li
                class="searchbox-list-li"
                v-for="content in search_content"
                :key="content.dataid"
                @click="showsearchcontent(content.dataid)"
              >{{content.title}}</li>
            </ul>
          </div>
        </div>
        <div id="content" :style="content_attribute">
          ...
          <div id="content-tag">
            <ul>
              <li v-for="tag in showTag" :key="tag.id" class="content-tag-li">
                <input
                  type="checkbox"
                  name="tag"
          ...
          <editor ref="editor" id="content-editor"></editor>
...
export default {
  ...
  methods: {
    ...
    getHeight() {
      ...
      this.catalogue_attribute.width =
        (window.innerWidth / 50) * 0.13 - 0.08 + "rem";
      this.search_attribute.width =
        (window.innerWidth / 50) * 0.104 - 0.08 + "rem";
      this.content_attribute.width =
        (window.innerWidth / 50) * 0.766 - 0.08 + "rem";
    },
    turnBlue($index) {
    ...
    newtag() {
      ...
      document.getElementById("content-tag-newtag-save").style.display =
        "inline";
    },
    newtagsave() {
    ...
    if (document.cookie.length == 0) {
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          while (tag.indexOf("_", pos1) != -1 && pos1 <= tag.length) {
            pos2 = tag.indexOf("_", pos1);
            nextthis.User.tag_list.push({
              id: tid,
              text: tag.slice(pos1, pos2)
            });
            pos1 = pos2 + 1;
            tid++;
          }
